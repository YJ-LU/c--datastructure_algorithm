...
void Set<T>::rotateRight(Elem *& node) {
	// you fill in here
    Elem * newroot = node->left;
	node->left = newroot->right;
	newroot->right = node;
	node->height = max(height(node->left), height(node->right)) + 1;
	newroot->height = max(height(node), height(newroot->left)) + 1;
	node = newroot;
}
...
void Set<T>::rotateLeft(Elem *& node){
    // you fill in here
    Elem * newroot = node->right;
    node->right = newroot->left;
    newroot->left = node;
    node->height = max(height(node->left), height(node->right)) + 1;
	newroot->height = max(height(node), height(newroot->right)) + 1;
    node = newroot;
}
